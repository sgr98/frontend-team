...
const ContributorRanker = ({ target }) => {
  ...
  function scaleValue(
    ...
    r1 = [
      0,
      data.winners[0].weekly.a *
        Math.ceil(data.winners[0].weekly.c / data.total_c),
    ],
    r2 = [10, 5000]
  ) {
    return Math.floor(
        ...
        <div className="center">
          ...
          <div className="top3">
            <a className="two item" href={data.winners[1].url} target="_blank" rel="noopener noreferrer">
              ...
              <div
                className="pic"
                style={{ backgroundImage: `url(${data.winners[1].img})` }}
              />
              <div className="name">{data.winners[1].id}</div>
              <div className="score">
                {scaleValue(
                  data.winners[1].weekly.a *
                    Math.ceil(data.winners[1].weekly.c / data.total_c)
                )}
              </div>
            </a>
            <a className="one item" href={data.winners[0].url} target="_blank" rel="noopener noreferrer">
              ...
              <div
                className="pic"
                style={{ backgroundImage: `url(${data.winners[0].img})` }}
              />
              <div className="name">{data.winners[0].id}</div>
              <div className="score">
                {scaleValue(
                  data.winners[0].weekly.a *
                    Math.ceil(data.winners[0].weekly.c / data.total_c)
            ...
            >
              ...
              <div
                className="pic"
                style={{ backgroundImage: `url(${data.winners[2].img})` }}
              />
              <div className="name">{data.winners[2].id}</div>
              <div className="score">
                {scaleValue(
                  data.winners[2].weekly.a *
                    Math.ceil(data.winners[2].weekly.c / data.total_c)
                )}
              </div>
            </a>
          </div>
            ...
            {data.authors.map((author, index) => (
              <a href={author.url} key={index} target="_blank" rel="noopener noreferrer">
                <div className="item">
                  ...
                  />
                  <div className="name">{author.id}</div>
                  <div className="score">{author.total}</div>
                </div>
              </a>
            ))}
          </div>
